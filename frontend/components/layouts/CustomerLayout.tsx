...
export default function CustomerLayout({ children, pathname }: CustomerLayoutProps) {
      ...
      <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-white shadow-lg transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-200 ease-in-out lg:translate-x-0  lg:inset-0`}>
        <div className="flex flex-col h-full">
          ...
          <div className="flex items-center justify-between h-16 px-6 bg-gradient-to-r from-green-600 to-green-700 text-white">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-xs text-green-200">Customer Account</p>
              </div>
            </div>
            <button 
              onClick={() => setSidebarOpen(false)}
              className="lg:hidden p-1 rounded-md hover:bg-green-800 transition-colors"
            >
          ...
          <nav className="flex-1 px-4 py-6 space-y-2">
            {customerNavigation.map((item) => {
              ...
              return (
                ...
                >
                  <svg className={`mr-3 h-5 w-5 ${isActive ? 'text-green-600' : 'text-gray-400 group-hover:text-gray-600'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="px-4 mb-4">
            <div className="bg-gradient-to-r from-green-50 to-green-100 rounded-xl p-4 border border-green-200">
              ...
              <div className="space-y-2">
                <Link href="/dashboard/claims/new" className="flex items-center text-sm text-green-700 hover:text-green-900 transition-colors">
                  <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                  </svg>
                  Submit New Claim
                </Link>
                <Link href="/dashboard/policies" className="flex items-center text-sm text-green-700 hover:text-green-900 transition-colors">
                  <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="border-t border-gray-200 p-4">
            ...
            <button
              onClick={logout}
              className="w-full flex items-center justify-center px-4 py-2 text-sm font-medium text-green-600 bg-green-50 rounded-lg hover:bg-green-100 transition-colors"
            >
              <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
