...
export default function AdminLayout({ children, pathname }: AdminLayoutProps) {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-white shadow-lg transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-200 ease-in-out lg:translate-x-0 lg:inset-0`}>
        <div className="flex flex-col h-full">
          {/* Logo */}
          <div className="flex items-center justify-between h-16 px-6 bg-gradient-to-r from-red-600 to-red-700 text-white">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-xs text-red-200">Insurance System</p>
              </div>
            </div>
            <button 
              onClick={() => setSidebarOpen(false)}
              className="lg:hidden p-1 rounded-md hover:bg-red-800 transition-colors"
            >
          ...
          <nav className="flex-1 px-4 py-6 space-y-2">
            {adminNavigation.map((item) => {
              ...
              return (
                ...
                >
                  <svg className={`mr-3 h-5 w-5 ${isActive ? 'text-red-600' : 'text-gray-400 group-hover:text-gray-600'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
